...
export async function launchNewTerminal(
	...
	// Security: validate folderPath if provided from untrusted webview message
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Folder path "${folderPath}" is outside the current workspace. Falling back to default.`);
