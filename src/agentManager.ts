...
export async function launchNewTerminal(
	...
	// Security: Validate folderPath is within workspace to prevent path traversal or unauthorized execution
	if (folderPath) {
		const uri = vscode.Uri.file(folderPath);
		if (!vscode.workspace.getWorkspaceFolder(uri)) {
			vscode.window.showWarningMessage(`Pixel Agents: folderPath "${folderPath}" is outside the current workspace. Falling back to default.`);
