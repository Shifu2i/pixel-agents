...
export async function launchNewTerminal(
	...
	// Security: validate that folderPath is within current workspace
	if (folderPath) {
		const folderUri = vscode.Uri.parse(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`[Pixel Agents] Security: Requested folder "${folderPath}" is not within workspace. Falling back to default.`);
