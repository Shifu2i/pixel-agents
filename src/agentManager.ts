...
export async function launchNewTerminal(
	...
	let validFolderPath: string | undefined = undefined;
	if (folderPath && folders && folders.length > 0) {
		const isInsideWorkspace = folders.some(folder => {
			const relative = path.relative(folder.uri.fsPath, folderPath);
			return relative === '' || (!relative.startsWith('..') && !path.isAbsolute(relative));
		});
		if (isInsideWorkspace) {
			validFolderPath = folderPath;
		} else {
	...
	const cwd = validFolderPath || folders?.[0]?.uri.fsPath;
