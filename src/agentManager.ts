...
export async function launchNewTerminal(
	...
	// Security: Validate folderPath is within workspace
	if (folderPath) {
		try {
			const folderUri = vscode.Uri.file(folderPath);
			const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
			if (!workspaceFolder) {
				console.warn(`[Pixel Agents] Security: Folder path ${folderPath} is outside workspace. Falling back to default.`);
				vscode.window.showWarningMessage(`Pixel Agents: Folder path ${folderPath} is outside workspace. Falling back to default.`);
				cwd = folders?.[0]?.uri.fsPath;
			}
		} catch (err) {
			console.error(`[Pixel Agents] Error validating folder path: ${err}`);
			cwd = folders?.[0]?.uri.fsPath;
		}
	}
