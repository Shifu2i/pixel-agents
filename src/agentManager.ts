...
export async function launchNewTerminal(
	...
	// Security: Validate folderPath is within the current workspace to prevent path traversal
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			console.warn(`[Pixel Agents] Security Warning: Attempted to launch terminal in path outside workspace: ${folderPath}`);
			vscode.window.showWarningMessage(`Pixel Agents: Cannot launch terminal outside workspace. Defaulting to workspace root.`);
