...
export async function launchNewTerminal(
	...
	// Security: Validate folderPath is within the active workspace to prevent path traversal
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			console.warn(`[Pixel Agents] Security: Requested folder "${folderPath}" is outside the workspace. Falling back.`);
			vscode.window.showWarningMessage(`Pixel Agents: Folder "${folderPath}" is outside the active workspace. Using primary folder instead.`);
