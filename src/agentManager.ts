...
export async function launchNewTerminal(
	...
	if (folderPath && folders && folders.length > 0) {
		const referenceUri = folders[0].uri;
		const targetUri = referenceUri.with({ path: folderPath });
		const isWithinWorkspace = vscode.workspace.getWorkspaceFolder(targetUri);
