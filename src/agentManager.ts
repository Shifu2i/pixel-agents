...
export async function launchNewTerminal(
	...
	// Security check: ensure folderPath is within the current workspace to prevent path traversal
	if (folderPath) {
		const uri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(uri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: folder "${folderPath}" is not in the workspace. Using default folder.`);
