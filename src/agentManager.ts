...
export async function launchNewTerminal(
	...
	// Security: Validate that the provided folderPath is within the current workspace
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Selected folder "${folderPath}" is outside the current workspace. Using default workspace folder.`);
