...
export async function launchNewTerminal(
	...
	// Validate that folderPath is within the current workspace to prevent path traversal
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Selected folder "${folderPath}" is not within the current workspace. Falling back to default.`);
