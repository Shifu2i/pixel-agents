...
export async function launchNewTerminal(
	...
	// Validate folderPath is within workspace to prevent path traversal/unauthorized access
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Selected folder is outside the active workspace. Falling back to workspace root.`);
