...
export async function launchNewTerminal(
	...
	// Security: Validate that folderPath is within the current workspace
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			console.warn(`[Pixel Agents] Security: Attempted to launch terminal in path outside workspace: ${folderPath}`);
			vscode.window.showWarningMessage(`Pixel Agents: Cannot launch agent in folder outside workspace.`);
