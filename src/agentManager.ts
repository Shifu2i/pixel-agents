...
export async function launchNewTerminal(
	...
	// Security: Validate that folderPath belongs to the current workspace to prevent path traversal
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Path "${folderPath}" is not within the current workspace. Falling back to default.`);
