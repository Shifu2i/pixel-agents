...
export async function launchNewTerminal(
	...
	// Security: Validate that folderPath is within the workspace
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			console.warn(`[Pixel Agents] Security: Requested folderPath "${folderPath}" is not within the current workspace.`);
			vscode.window.showWarningMessage(`Pixel Agents: Folder "${folderPath}" is outside the workspace. Using default folder.`);
