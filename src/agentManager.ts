...
export async function launchNewTerminal(
	...
	// Security: Validate folderPath if provided by the webview to prevent path traversal
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Invalid folder path "${folderPath}". Terminals can only be opened within the current workspace.`);
			return;
		}
	}
