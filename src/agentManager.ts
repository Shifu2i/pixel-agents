...
export async function launchNewTerminal(
	...
	// Security check: ensure folderPath is within a workspace folder
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			console.warn(`[Pixel Agents] Security: blocked attempt to open terminal in non-workspace path: ${folderPath}`);
