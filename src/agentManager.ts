...
export async function launchNewTerminal(
	...
	// Security: Validate that folderPath is within workspace folders to prevent unauthorized terminal launches
	if (folderPath && folders) {
		const isWithinWorkspace = folders.some((f) => {
			const rel = path.relative(f.uri.fsPath, folderPath);
			return rel === '' || (!rel.startsWith('..') && !path.isAbsolute(rel));
		});
		if (!isWithinWorkspace) {
			console.warn(`[Pixel Agents] Security: blocked terminal launch in unauthorized path: ${folderPath}`);
