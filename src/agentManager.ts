...
export async function launchNewTerminal(
	...
	let validatedCwd: string | undefined = undefined;

	if (folderPath) {
		const activeFolders = folders || [];
		const isWithinWorkspace = activeFolders.some(folder => {
			const workspaceFsPath = folder.uri.fsPath;
			const relative = path.relative(workspaceFsPath, folderPath);
			return relative === '' || (!relative.startsWith('..') && !path.isAbsolute(relative));
		});

		if (isWithinWorkspace) {
			validatedCwd = folderPath;
		} else {
	...
	const cwd = validatedCwd || folders?.[0]?.uri.fsPath;
