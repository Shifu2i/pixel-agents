...
export async function launchNewTerminal(
	...
	if (folderPath && folders) {
		...
		if (!validCwd) {
			console.warn(`[Pixel Agents] Provided path "${folderPath}" is not within workspace. Falling back to default.`);
		}
	}
	const cwd = validCwd || folders?.[0]?.uri.fsPath;
