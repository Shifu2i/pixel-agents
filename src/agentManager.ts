...
export async function launchNewTerminal(
	...
	if (folderPath) {
		...
		if (!workspaceFolder) {
			console.warn(`[Pixel Agents] Attempted to launch terminal in path outside workspace: ${folderPath}`);
			vscode.window.showWarningMessage(`Pixel Agents: folder path "${folderPath}" is outside the current workspace. Defaulting to first workspace folder.`);
