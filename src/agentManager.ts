...
export async function launchNewTerminal(
	...
	// if an attacker tries to pass an arbitrary path via webview messages.
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			console.warn(`[Pixel Agents] Security: Provided path "${folderPath}" is not within the workspace. Falling back to default.`);
