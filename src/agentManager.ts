...
export async function launchNewTerminal(
	...
	// Security: Validate that the provided folderPath is within the current workspace to prevent path traversal
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Terminal launch blocked for path outside workspace: ${folderPath}`);
