...
export async function launchNewTerminal(
	...
	// Security: Validate that folderPath belongs to the current workspace
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Invalid workspace path "${folderPath}". Defaulting to workspace root.`);
