...
export async function launchNewTerminal(
	...
	// Security: Validate user/webview supplied folderPath against active workspace folders
	let safeFolderPath = folderPath;
	if (safeFolderPath && folders && folders.length > 0) {
		const isWithinWorkspace = folders.some(folder => {
			const relative = path.relative(folder.uri.fsPath, safeFolderPath!);
		...
		if (!isWithinWorkspace) {
			...
			safeFolderPath = undefined;
		}
	}

	const cwd = safeFolderPath || folders?.[0]?.uri.fsPath;
