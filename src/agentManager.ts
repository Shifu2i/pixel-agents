...
export async function launchNewTerminal(
	...
	// Security: Validate folderPath belongs to the active workspace to prevent path traversal
	if (folderPath) {
		const folderUri = vscode.Uri.file(folderPath);
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(folderUri);
		if (!workspaceFolder) {
			console.warn(`[Pixel Agents] Security: Requested folderPath is outside workspace: ${folderPath}`);
			vscode.window.showWarningMessage(`Pixel Agents: Requested folder is outside the active workspace. Defaulting to workspace root.`);
