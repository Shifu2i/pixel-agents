...
export async function launchNewTerminal(
	...
	// Validate folderPath against active workspace folders to prevent path traversal / launching terminals in arbitrary directories
	if (folderPath && folders && folders.length > 0) {
		const isWithinWorkspace = folders.some((folder) => {
			const relative = path.relative(folder.uri.fsPath, folderPath);
			return !relative.startsWith('..') && !path.isAbsolute(relative);
		});

		if (!isWithinWorkspace) {
			console.warn(`[Pixel Agents] Security warning: Provided path "${folderPath}" is not within any workspace folder.`);
