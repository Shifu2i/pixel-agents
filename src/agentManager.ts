...
export async function launchNewTerminal(
	...
	// SECURITY: Validate folderPath from untrusted webview message to prevent path traversal
	if (folderPath) {
		let isWithinWorkspace = false;
		if (folders) {
			isWithinWorkspace = folders.some(folder => {
				const rel = path.relative(folder.uri.fsPath, folderPath);
