...
export async function launchNewTerminal(
	...
	// Security: Validate folderPath is within workspace to prevent path traversal/unauthorized access
	if (folderPath && folders) {
		const workspaceFolder = vscode.workspace.getWorkspaceFolder(vscode.Uri.file(folderPath));
		if (!workspaceFolder) {
			vscode.window.showWarningMessage(`Pixel Agents: Folder "${folderPath}" is outside the active workspace. Using default folder.`);
